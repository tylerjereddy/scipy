...
- A new public base class `scipy.sparse.sparray` was introduced, allowing further
  extension of the sparse array API (such as the support for 1- and
  n-dimensional sparse arrays) without breaking backwards compatibility.
  ...
  matrix classes.
- Division of sparse arrays by a dense array now return sparse arrays.
- `scipy.sparse.isspmatrix` now only return `True` for the sparse matrices instances.
